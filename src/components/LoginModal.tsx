      ...
      <div className="bg-white rounded-2xl w-full max-w-md overflow-hidden shadow-2xl animate-in fade-in zoom-in duration-200">
        <div className="p-6 relative">
          ...
          <div className="mt-4 pt-4 border-t border-gray-100 text-center">
             <p className="text-xs text-gray-400 mb-2">Dành cho demo/kiểm thử:</p>
             <button 
               onClick={() => {
                 // Mock Admin Login
                 setLoading(true);
                 setTimeout(() => {
                   ...
                   api.post('/api/sync-user', {
                     ...
                     // Note: Real backend won't make this user admin automatically unless we force it or run SQL
                     // But for frontend UI testing, we can try. 
                     // Actually, let's just log them in and tell them to run SQL.
                     setLoading(false);
                     alert('Đăng nhập thành công! Để có quyền Admin thực sự, bạn cần chạy lệnh SQL trong D1: UPDATE users SET role = "admin" WHERE encrypted_yw_id = "' + mockUserId + '"');
                     window.location.reload();
                   });
                 }, 1000);
               }}
             ...
             >
               Đăng nhập nhanh với vai trò Admin (Cần set DB)
             </button>
          </div>
        </div>
      </div>
