  ...
  async fetch(request: Request, env: Env, ctx: ExecutionContext): Promise<Response> {
    ...
    if (url.pathname === '/api/stories' && method === 'GET') {
      ...
      const parsedResults = results.map((story) => ({
        ...story,
        genres: typeof story.genres === 'string' ? JSON.parse(story.genres) : story.genres
      }));
    ...
    if (url.pathname === '/api/user/me' && method === 'GET') {
      ...
      return new Response(JSON.stringify(user || { balance: 0, earnings: 0, role: 'reader' }), { headers });
    }
